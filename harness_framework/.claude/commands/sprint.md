...
QA 에이전트(test-builder, risk-reviewer, production-guard)는 `.claude/stack.md`와 `.claude/qa.md` 두 파일을 모두 읽어 동작합니다. `qa.md`가 없으면 `.claude/qa.md.template`에서 복사 후 채우도록 안내하세요.
