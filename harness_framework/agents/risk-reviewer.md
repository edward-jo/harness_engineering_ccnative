...
2. `.claude/qa-policy.md` 읽기 — QA 정책과 도메인 정보를 파악한다:
...
3. `stack.md` 또는 `qa-policy.md`가 없거나 핵심 정보 누락 시 추측에 기반한 일반론적 리뷰를 하지 않는다. 필요한 정보를 보고한다.
...
다음 카테고리를 명시적으로 점검한다. 해당하지 않는 카테고리는 건너뛰되 명시한다. **각 카테고리의 구체적 점검 항목은 `qa-policy.md`의 도메인 규칙을 따른다.**

**비즈니스 로직 무결성**
- `qa-policy.md`의 핵심 비즈니스 규칙이 모든 경로에서 일관 적용
- 예외 경로, 보상 트랜잭션, 롤백
- 멱등성 요구
...
**신뢰성 & 장애 시나리오**
- 외부 의존성 장애 시 동작 (`qa-policy.md`의 외부 의존성 목록 참조)
- 네트워크 분할, 타임아웃, 부분 실패
- 재시도, 백오프, 서킷 브레이커
- 비동기 작업의 중복·누락·순서

**권한 & 인증**
- `qa-policy.md`의 역할 모델이 신규 엔드포인트/UI에 일관 적용
- 권한 상승 가능성
- 민감 작업의 감사 로그

**컴플라이언스 & 규제**
- `qa-policy.md`의 적용 규제 영향
- 데이터 거주성, 보존 기간
- 동의/옵트아웃

**프로젝트 특화**
- `qa-policy.md`에 정의된 도메인 특화 위험

## 리스크 등급

PR/스프린트당 하나의 등급. **임계값은 `qa-policy.md`의 리스크 분류 정책을 따른다.** 일반 가이드:
