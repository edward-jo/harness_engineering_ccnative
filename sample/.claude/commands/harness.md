...
## 모드 1: 새 project 시작 — `$ARGUMENTS`가 비어있거나 `list`/`extend`/`finish`가 아닌 자유 텍스트
...
2. **비어있지 않으면 중단**하고 다음을 안내:
   ...
   - 기존 project를 종료하고 새로 시작하려면: /harness finish 후 다시 /harness <아이디어>
   ```
...
## 모드 4: `list` — project 나열

1. `archive/sprints/*/META.json`을 순회하며 `{slug, title, started, finished, sprint_count}` 추출.
