#!/bin/bash
# Stop 훅: sprint_result.json 확인 후 FAIL이면 Claude를 재실행시키는 루프 가드
...
# sprint_result.json 확인 (evaluator가 기록)
if [ -f "sprint_result.json" ]; then
  ...
  if [ "$STATUS" = "FAIL" ]; then
    ...
    jq -n \
      --arg reason "[$NEW_COUNT/$MAX_LOOPS] 스프린트 $SPRINT 검증 실패 ($PASSED/$TOTAL 통과). 실패 항목: $FAILS. generator 에이전트로 수정 후 evaluator로 재검증하세요." \
      '{
        "decision": "block",
        "reason": $reason
      }'
