...
- **E2E pair (e2e-author · e2e-runner-reporter, v2.3+)**: adoption 트랙 전용. e2e-author가 `feature_inventory.json` 기반 무인 모드로 qa-policy의 `e2e_tool`(Playwright/Maestro/Cypress 등)의 spec 파일을 생성하고, e2e-runner-reporter가 spec을 실행해 실패한 시나리오를 GitHub issue로 자동 등록한다 (dedup: label+feat-id). v2.6+에서는 screenshot을 orphan `e2e-assets` 브랜치에 push해 issue 본문에 markdown image로 embed (GitHub 웹에서 직접 미리보기 가능).
